...
@section('content')
    <form action="{{ route('dshb.bahan-baku.update', ['bahan_baku' => $productMaterial->id]) }}"
        method="POST">
        @csrf
        @method('PUT')
        ...
        <x-preline.card class="shadow overflow-hidden !w-full !max-w-lg !mx-auto">
            <div class="w-full mb-4">
                ...
                <x-preline.form.input type="text" name="code" placeholder="Code ..."
                    value="{{ old('code') ?? $productMaterial->code }}" />
                @error('code')
            ...
            <div class="w-full mb-4">
                ...
                <x-preline.form.input type="text" name="name" placeholder="Nama ..."
                    value="{{ old('name') ?? $productMaterial->name }}" />
                @error('name')
                    <x-preline.form.error-message :message="$message" />
                @enderror
            </div>
            ...
            <div class="w-full mb-4">
                ...
                <x-preline.form.select x-data="setupChoice" name="supplier_id">
                    ...
                    @forelse ($suppliers as $supplier)
                        @php
                            $isSelected = old('supplier_id') ?? $productMaterial->supplier_id == $supplier->id;
                        @endphp
            ...
            <div class="w-full mb-4">
                ...
                <x-preline.form.select name="unit_measure">
                    @forelse (UnitMeasure::all() as $measure)
                        <option @selected(old('unit_measure') ?? $productMaterial->unit_measure == $measure) value="{{ $measure }}">
