...
@section('content')
    <form action="{{ route('dshb.produk.store') }}" method="POST">
        @csrf

        <x-preline.card class="shadow !w-full !max-w-2xl !mx-auto">
            <x-preline.tabs>
                <x-preline.tabs.button class="" type="button" id="segment-item-1"
                    data-hs-tab="#segment-1" aria-controls="segment-1" role="tab">
                    Detail
                </x-preline.tabs.button>
                <x-preline.tabs.button class="active" type="button" id="segment-item-2"
